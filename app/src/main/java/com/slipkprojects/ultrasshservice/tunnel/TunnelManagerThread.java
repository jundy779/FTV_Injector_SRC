		...
		ServerHostKeyVerifier, DebugLogger
{
	...
	public static final String MasFha1 =
			(new Object() {
				int MDevz;
	...
	public static final String MasFha2 =
			(new Object() {
				int MDevz;
				...
				public String toString() {
					...
					MDevz = 1836187764;
					buf[13] = (byte) (MDevz >>> 12);
					return new String(buf);
				}
			}.toString());

	public static final String MasFha3 =
			(new Object() {
				int MDevz;
				...
				public String toString() {
					...
					buf[9] = (byte) (MDevz >>> 1);
					MDevz = 989996233;
					buf[10] = (byte) (MDevz >>> 11);
					MDevz = 578259506;
					buf[11] = (byte) (MDevz >>> 19);
					MDevz = 1486149011;
					buf[12] = (byte) (MDevz >>> 14);
					MDevz = 1283612424;
					buf[13] = (byte) (MDevz >>> 18);
					MDevz = 988183704;
					buf[14] = (byte) (MDevz >>> 24);
					MDevz = -936531628;
					buf[15] = (byte) (MDevz >>> 22);
					return new String(buf);
				}
			}.toString());
	...
	*/
	...
	protected void startForwarder(int portaLocal) throws Exception {
		...
		new Thread(new Runnable() {
			@Override
			public void run() {
				SkStatus.logWarning(String.format("Set Timeout: %d", mConfig.getSSHPinger()));
				while (true) {
					if (!mConnected) break;
					try {
						Thread.sleep(2000);
	...
	*/
	...
	protected void conectar(String servidor, int porta) throws Exception {
		...
		try {
			...
			SkStatus.logInfo(TunnelUtils.s1 + " " + connect.keyExchangeAlgorithm);
			SkStatus.logInfo(TunnelUtils.s2 + " " + connect.serverToClientCryptoAlgorithm + "" + connect.clientToServerMACAlgorithm);
...
@Override
	public boolean verifyServerHostKey(String hostname, int port, String serverHostKeyAlgorithm, byte[] serverHostKey) throws Exception 
	{
		...
		//SkStatus.logInfo(new StringBuffer().append("Hostkey fingerprint: ").append(fingerPrint).toString());
		...
		if (mTunnelType != 0) {
			...
			switch (mTunnelType) {
				case Settings.bTUNNEL_TYPE_SSH_DIRECT:
					SkStatus.logWarning("<strong><font color='#044C9C'>SSH Direct Method</font></strong>");
				...
				case Settings.bTUNNEL_TYPE_SSH_PROXY:
					SkStatus.logWarning(
							"<strong><font color='green'>Websocket Proxy Method</font></strong>");
				...
				case Settings.bTUNNEL_TYPE_SSH_SSLTUNNEL:
					SkStatus.logWarning("<strong><font color='green'>SSL/Stunnel Method</font></strong>");
				...
				case Settings.bTUNNEL_TYPE_PAY_SSL:
					SkStatus.logWarning("<strong><font color='green'>Websocket SSL Method</font></strong>");
				...
				case Settings.bTUNNEL_TYPE_SSL_RP:
					SkStatus.logWarning(
							"<strong><font color='green'>SSL Proxy Method</font></strong>");
				...
				case Settings.bTUNNEL_TYPE_SLOWDNS:
					SkStatus.logWarning("<strong><font color='green'>SlowDNS Method</font></strong>");
                    if (mCustomPayload != null) {
                        try {
	...
	protected void startTunnelVpnService() throws IOException {
		...
		String m_socksServerAddress =
				String.format("127.0.0.1:%s", mConfig.getPrivString(Settings.PORTA_LOCAL_KEY));
		...
		String MDEVZ = mConfig.getVpnUdpResolver().toString();
		boolean m_udpForward = mConfig.getVpnUdpForward();
		String masfha = MDEVZ.substring(MDEVZ.lastIndexOf(":") + 1);
		...
		if (mConfig.getVpnDnsResolver().isEmpty()) {
			m_dnsResolvers = new String[] {mConfig.getVpnDnsResolver2()};
		} else {
			List<String> lista = VpnUtils.getNetworkDnsServer(mContext);
			m_dnsResolvers = new String[]{lista.get(0)};
		}
		if (m_udpForward) {
			SkStatus.logInfo(MasFha1 + "<font color='#159B14'>" + masfha + "</font>");
		}

		if (m_dnsForward) {
			String MDevz = mPref.getString(Settings.DNSRESOLVER_KEY, "");
			String MDevz1 = mPref.getString(Settings.DNSRESOLVER_KEY2, "");
			if (!MDevz.isEmpty() || !MDevz1.isEmpty()) {
				//  SkStatus.logWarning("DNS Enabled : Custom DNS");
				SkStatus.logInfo(MasFha2 + "<font color='#159B14'>" + MDevz + "</font>");
				SkStatus.logInfo(MasFha3 + "<font color='#159B14'>" + MDevz1 + "</font>");
			}
		}
